...
* Актуальный проект, состоящий из 2 модулей (appForge и appNeoForge). В этом случае папкой проекта является весь
  репозиторий целиком.
* Архивный проект, состоящий из 13 модулей (1.3.2 - 1.7.10, 1.12.2 - 1.19.4). То есть, папкой проекта
  является `archive1`.
* Устаревший архив, состоящий из 4 отдельных проектов (1.8.9 - 1.11.2). Здесь папками проекта являются четыре папки
  внутри `archive2`.

Таким образом, в общей сложности, этот репозиторий содержит в себе шесть независимых проектов, которые открываются
отдельно и редактируются отдельно. Про то, как открывать проекты и работать с ними, будет подробно рассказано ниже.

| Версия Minecraft | Gradle | Плагин сборки            | Gradle JDK | Project JDK | Toolchain JDK | Syntax |
|------------------|--------|--------------------------|------------|-------------|---------------|--------|
| 1.3.2 - 1.6.4    | 8.5    | Voldeloom 2.5-SNAPSHOT   | 21.0.2     | 21.0.2      | 11.0.21       | 6      |
| 1.7.10, 1.12.2   | 8.5    | RetroFuturaGradle 1.3.30 | 21.0.2     | 21.0.2      | 1.8.0_392     | 8      |
...
| 1.13.2 - 1.16.5  | 8.5    | ForgeGradle 6.0.16,6.2   | 21.0.2     | 21.0.2      | 1.8.0_392     | 8      |
| 1.17.1           | 8.5    | ForgeGradle 6.0.16,6.2   | 21.0.2     | 21.0.2      | 16.0.2        | 16     |
| 1.18.2 и новее   | 8.5    | ForgeGradle 6.0.16,6.2   | 21.0.2     | 21.0.2      | 17.0.9        | 17     |
...
должны увидеть несколько папок: `appForge`, `appNeoForge`, `archive1`, `archive2` и так далее.
...
Как уже было сказано ранее, **весь репозиторий** - это один проект; **archive1** - второй проект; папки внутри
**archive2** - ещё четыре проекта. Любой из этих шести проектов можно установить и открыть в IntrlliJ IDEA, и в таком
случае вы получите возможность редактировать исходный код для тех или иных версий игры.

* Если вас интересует разработка под 1.20.4, ваша цель - весь репозиторий.
* Если вас интересует разработка под более старые версии, ваша цель - archive1.
* Некоторые версии невозможно открыть скопом, и их придётся открывать по отдельности. Они лежат в archive2.

Образцы рассуждений:

* Я хочу отредактировать мод для новейшей версии. Значит, папка проекта - весь репозиторий.
* Я хочу отредактировать мод для 1.16.5. 1.16.5 лежит в `archive1`. Значит, папка проекта - `archive1`.
* Я хочу отредактировать мод для 1.8.9. 1.8.9 лежит в `archive2`. Значит, папка проекта - `1.8.9`.

Теперь, когда вы знаете, какая папка проекта вас интересует, осталось определить, какой плагин сборки используется для
выбранной версии игры.
