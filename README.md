...
* Версии 1.8.9, 1.9.4, 1.10.2 и 1.11.2 используют устаревший ForgeGradle, который всё ещё жив в 2023 году. Однако есть проблема - среды сами не установятся, если импортировать папку проекта в Eclipse или открыть через IntelliJ IDEA. Чтобы среда сформировалась, нужно запустить один из двух батников в папке - setupEclipse или setupIntellij.
  ...
  * Я умышленно не упомянул здесь 1.7.10, потому что там есть два варианта - установка автоматическая, как на новых версиях, и использование батника. Там и там есть свои плюсы - в первом случае Gradle Tasks, во втором запуск из среды. Однако ведь никто не запрещает сначала пойти первым сценарием, а затем выполнить второй, чтобы он просто доскачал нужные файлы.
