...
public class LegendariumArmor
    ...
    public String getArmorTexture(ItemStack itemstack, Entity entity, int slot, String type) {
        String texture = "legendarium:armor/" + LegendariumCommander.getArmorName(this);
        if (type != null) {
    ...
    private static LOTRMaterial newLOTRMaterial(String name, int uses, float damage, float protection, float speed, int harvestLevel, int enchantability, Item craftingMaterialTool, Item craftingMaterialArmor) {
        return LegendariumArmor.newLOTRMaterial(name, uses, damage, protection, speed, harvestLevel, enchantability, craftingMaterialTool, craftingMaterialArmor, false, false);
    }
    ...
    private static LOTRMaterial editLOTRMaterial(LOTRMaterial material, int uses, float damage, float protection, float speed, int harvestLevel, int enchantability, Item craftingMaterialTool, Item craftingMaterialArmor, boolean manFlesh, boolean undamageable) {
        ...
        try {
            ...
            if (craftingMaterialTool != null && craftingMaterialArmor != null) {
                setCraftingMaterial.invoke((Object)material, new Object[]{craftingMaterialTool, craftingMaterialArmor});
            }
            if (undamageable) {
                setUndamageable.invoke((Object)material, new Object[0]);
            }
            if (manFlesh) {
                setManFlesh.invoke((Object)material, new Object[0]);
            }
        }
    ...
    private static void setup() {
        ...
        try {
            ...
            setUndamageable = clazz.getDeclaredMethod("setUndamageable", new Class[0]);
            setUndamageable.setAccessible(true);
            setManFlesh = clazz.getDeclaredMethod("setManFlesh", new Class[0]);
    ...
    private static void setExistingCraftingItem(LOTRMaterial material, Item item) {
        LegendariumArmor.setExistingCraftingItem(material, new ItemStack(item), item);
    }
